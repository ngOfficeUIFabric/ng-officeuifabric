 ...
 */
export enum UifChoicefieldType {
    radio,
    checkbox
}
 ...
 *
 * 
 * @property {string} ngModel               (optional) - Set the ng-model
 * @property {string} ngTrueValue           (optional) - Set the ng-true-value. Can only be used with checkbox
 * @property {string} ngFalseValue          (optional) - Set the ng-false-value. Can only be used with checkbox
 * @property {boolean} disabled             (optional) - Set to disable the element
 * @property {uifChoicefieldType} uifType   (optional) - Radio or Checkbox
 */
    ...
    private preLink(
        ...
        transclude: ng.ITranscludeFunction): void {
            ...
            scope.$watch('uifType', (newValue: string, oldValue: string) => {
                if (UifChoicefieldType[newValue] === undefined) {
                    choicefieldOptionController.$log.error('Error [ngOfficeUiFabric] officeuifabric.components.choicefield - "' +
                        newValue + '" is not a valid value for uifType. It should be radio or checkbox');
                }
            });
