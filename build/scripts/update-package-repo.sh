...
function init {
  # setup paths
  SRC_PATH=$SRCWD
  PKG_PATH=$PKGWD/package-ngofficeuifabric
  # setup globals
...
function run {
  ...
  cp -Rf $SRC_PATH/dist/*.js $PKG_PATH
  cp -Rf $SRC_PATH/CHANGELOG.md $PKG_PATH/CHANGELOG.md


  # update versions & dependencies in ng-office-ui-fabric.nuspec
  echo "DEBUG: [3 / 4] updating versions & dependencies in package.json & bower.json"
  node $SRC_PATH/build/scripts/update-package-versions.js --src=$SRC_PATH --pkg=$PKG_PATH
