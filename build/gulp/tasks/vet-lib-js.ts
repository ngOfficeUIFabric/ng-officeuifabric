...
export class GulpTask extends BaseGulpTask {
  ...
  constructor(done: IVoidCallback) {
    ...
    let allJavaScript: string[] = [];

    // get all JS in the library except files that are explicitly excluded
    BuildConfig.LIB_JS.forEach((libJsPattern: string) => {
      // convert pattern to glob pattern
      allJavaScript.push(libJsPattern.replace('/*/', '/**/'));
    });
    // add to the glob pattern files that should be excluded
    allJavaScript = allJavaScript.concat(BuildConfig.LIB_EXCLUDE_LINT_JS);
